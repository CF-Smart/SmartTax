...
// ATENÇÃO: Toda vez que o usuário seleciona 'Pralog', o dashboard carrega e manipula os dados do usuário 'cfsmart'.
// Isso garante que a CF Smart é a administradora dos dados da Pralog, mantendo tudo centralizado.
// O branding e nome exibido mudam conforme o login, mas a fonte dos dados é sempre 'cfsmart'.
function App() {
  ...
  if (!isAuthenticated) {
    if (!selectedEmpresa) {
      return <EmpresaSelect onSelect={setSelectedEmpresa} />;
    }
    return <LoginForm onLogin={handleLogin} empresa={selectedEmpresa} onVoltar={() => setSelectedEmpresa(null)} />;
  }

  // Branding customizado para Pralog
  const isPralog = true; // Sempre Pralog agora
